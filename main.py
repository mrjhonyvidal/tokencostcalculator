...
if detailMode: 
    ...
    mean_queries = (values[0]+values[1])/2
    ...
    st.write(f'<p style="text-align:center;">Average cost would be (mean) <span style="color: rgb(255, 75, 75)"> {mean_cost:.2f}$</span> for <span style="color: rgb(255, 75, 75)"> {mean_queries}</span> queries based on (minimum total costs * max queries + minimum total costs * min queries + maximum total costs * max queries + maximum total costs * min queries) / 4</span></p>', unsafe_allow_html=True)
